...
def run_single_delta_gao(n,delta, K, linkage, num_trials=2000, base_seed=0):
    delta = float(delta)
    base_seed = int(base_seed)
    rng = np.random.default_rng(base_seed + int(delta * 1000))
    return compute_power_Gao(n,1, K, delta, 0.05, linkage,num_trials, rng=rng)
...
if __name__ == "__main__":
    ...
    with Pool(processes=num_workers, initializer=init_worker) as pool:
        results_gao = pool.starmap(run_single_delta_gao, [(n, delta, K, linkage, n_trials) for delta in deltas])

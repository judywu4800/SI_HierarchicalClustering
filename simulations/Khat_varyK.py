...
if __name__ == "__main__":
    ...
    K_hat_gap = gap_statistic_full(X,K_max=10, seed=K + 1000*args.trial+6)["K_hat"]

    out = pd.DataFrame([[args.K, args.trial, K_hat_F, K_hat_gap]],
                       columns=["K_true", "trial", "K_hat_F", "K_hat_gap"])

    os.makedirs(f"results/raw/fig11", exist_ok=True)
    out.to_csv(f"results/raw/fig11/K{args.K}_trial{args.trial}.csv",
               index=False)
    ...
    os.makedirs(f"results/raw/fig11/pvals", exist_ok=True)
    df.to_csv(f"results/raw/fig11/pvals/K{args.K}_trial{args.trial}.csv",)

...
def compute_pval_gao_es(X,true_means, K=3, linkage = "complete",seed = None):
    ...
    return result
def compute_power_Gao(n,sigma, K, delta, alpha, num_trials=10000, linkage="complete",rng=None):
    if rng is None:
    ...
    for _ in range(num_trials):
        ...
        pval, effect = np.array(compute_pval_gao_es(X,true_means, K=K, linkage=linkage, seed=trial_seed))
...
def run_single_delta_gao(n,delta,K,linkage, base_seed=0):
    ...
    return compute_power_Gao(n,1, K, delta, 0.05, 2000, linkage,rng=rng)

if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser()
    parser.add_argument("--linkage", type=str, required=True, choices=["single", "average"])
    ...
    print(f"\n=== Running Gao (sigma_clustered) ES linkage={linkage}, K={K}, num_trials={n_trials} ===")
    ...
    csv_path = os.path.join(output_dir, f"rejection_es_gao_clustered_K{K}_{linkage}.csv")

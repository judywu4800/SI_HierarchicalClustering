...
def compute_pval_barber_es(X,true_means, K=3, linkage = "complete"):
    ro.r('source("/home/judydw/RAC_invariant/r_functions.R")')
    #ro.r('source("/Users/judydw/Documents/GitHub/SI_HierarchicalClustering/r_functions.R")')
    ...
    result = ro.r("get_barber_pval_es(X,true_mean_py, K_py, link_py)")
    return result
def compute_power_barber(sigma, delta, alpha, num_trials=10000):
    p_values = []
    effect_sizes = []
    for _ in range(num_trials):
        X, true_label, true_means = generate_data_barbers(10, delta, sigma, true_mean = True)
        pval, effect = np.array(compute_pval_barber_es(X,true_means, K=3, linkage="complete"))
...
def run_single_delta_barber(delta):
    return compute_power_barber(1, delta, 0.05, 2000)
...
if __name__ == "__main__":
    ...
    output_dir = os.path.join(base_dir,"results", f"barber_es_results_{timestamp}")
    os.makedirs(output_dir, exist_ok=True)

    csv_path = os.path.join(output_dir, "rejection_and_effect_barber.csv")

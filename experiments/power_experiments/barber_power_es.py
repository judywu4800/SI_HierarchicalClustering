...
def compute_pval_barber_es(X,true_means, K=3, linkage = "complete",seed=None):
    ...
    return result
def compute_power_barber(sigma, K, delta, alpha, num_trials=10000, rng=None):
    if rng is None:
    ...
    for _ in range(num_trials):
        ...
        pval, effect = np.array(compute_pval_barber_es(X,true_means, K=K, linkage="complete", seed=trial_seed))
...
def run_single_delta_barber(delta,K,base_seed = 0):
    rng = np.random.default_rng(base_seed + int(delta * 1000))
    return compute_power_barber(1, K, delta, 0.05, 2000, rng=rng)
...
if __name__ == "__main__":
    ...
    with Pool(processes=num_workers, initializer=init_worker) as pool:
        results_gao = pool.starmap(run_single_delta_barber, [(delta, K) for delta in deltas])
    ...
    csv_path = os.path.join(output_dir, f"rejection_es_barber_K{K}.csv")

...
from find_best_K import find_best_K_chi, find_best_K_F, generate_alpha_list,generate_alpha_list_exp
...
def generate_random_covariance(p, scale=1.0, random_state=None):
    rng = np.random.default_rng(random_state)
    ...
    # Add a tiny ridge if needed (for numerical stability)
    min_eig = np.min(np.linalg.eigvalsh(Sigma))
    if min_eig <= 0:
        Sigma += np.eye(p) * (1e-6 - min_eig)

    return Sigma

def generate_3cluster_data_varsize(n=30, p=2, delta=1.0, sigma=1.0,
    ...
    if np.isscalar(sigma):
        ...
        if cov.shape != (p, p):
            raise ValueError(f"Sigma must have shape ({p},{p}), got {cov.shape}.")
    ...
    # Generate clusters
    # --------------------
    X1 = multivariate_normal.rvs(mean=mu1, cov=cov, size=cluster_sizes[0], random_state=rng)
    X2 = multivariate_normal.rvs(mean=mu2, cov=cov, size=cluster_sizes[1], random_state=rng)
    X3 = multivariate_normal.rvs(mean=mu3, cov=cov, size=cluster_sizes[2], random_state=rng)
...
def one_replication_std(delta,n=30, tau=0.1, total_alpha=0.05,
    ...
    alpha_list = generate_alpha_list_exp(n, total_alpha, decay_rate=1)
    #K_hat_F, _, _ = find_best_K_chi(X, sigma = sigma, tau=tau, alpha_list=alpha_list, total_alpha=total_alpha)
    K_hat_F, _, _, _ = find_best_K_F(X, tau=tau, alpha_list=alpha_list, total_alpha=total_alpha, n_threshold=25)
...
if __name__ == "__main__":
    ...
    results = simulate_results_std(delta_list, n_rep=30, n_jobs=-1)
    rows = []
    for sd, result in results.items():
        for kf, kg in zip(result["Proposed Method"], result["Gap Test"]):
            rows.append((sd, kf, kg))

    df = pd.DataFrame(rows, columns=["delta", "K_hat_F", "K_hat_gap"])

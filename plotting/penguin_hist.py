...
if __name__ =='__main__':
    ...
    cooccurrence = pd.read_csv(os.path.join(base_dir, "results/raw/cooccurrence.csv"), header=None).to_numpy()
    ...
    '''
        ax1.fill_between(
        x, F_y, 0, step="pre",
        alpha=0.7, color="#3A8E7A", edgecolor="black", label="Proposed Method"
    )
    ax1.fill_between(
        x, G_y, 0, step="pre",
        alpha=0.7, color="#6FB7E9", edgecolor="black", label="Gap Statistic"
    )
    ax1.axvline(3, color="red", linestyle="--", linewidth=2, label=r"$K^*$ = 3")

    ax1.set_xticks(np.arange(Kmin, Kmax + 1))
    ymax = max(counts_F.max(), counts_G.max())
    ax1.set_ylim(-1.1*ymax, 1.1*ymax)
    ax1.set_xlabel(r"$\widehat{K}$", fontsize=12)
    ax1.set_ylabel("Frequency", fontsize=12)
    yticks = ax1.get_yticks()
    ax1.set_yticklabels([f"{int(abs(y))}" for y in yticks])

    ax1.set_title(r"Paired Histogram of $\widehat{K}$", fontsize=16)
    ax1.legend()
    ...
    ax1.set_title(r"Histogram of Proposed $\widehat{K}$ with Gap Lines", fontsize=16)
